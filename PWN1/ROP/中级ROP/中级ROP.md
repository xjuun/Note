...
- 从0x400600到0x400609，可以将r13赋值给rdx，将r14赋值给rsi，嫁女r15赋值给edi（虽然这里赋值给edi，但其实此时rdi的高32位寄存器为0，所以我们其实只能控制低32位），而上述的3个寄存器其实就是x64函数调用时用到的前三个寄存器。**此外，如果我们可以合理地控制 r12 与 rbx，那么我们就可以调用我们想要调用的函数**，比如说我们可以控制 rbx 为 0，r12 为存储我们想要调用的函数的地址
